...
from django_drf_filepond.utils import (
    _get_file_id,
    _get_user,
...
class ProcessView(APIView):
    ...
    def post(self, request):
        ...
        if not hasattr(local_settings, "UPLOAD_TMP"):
            print(getattr(local_settings, "UPLOAD_TMP"))
            return Response(
...
class RevertView(APIView):
    ...
    def delete(self, request):
        ...
        try:
            ...
            LOG.debug(
                "About to delete temporary upload <%s> with original "
...
class LoadView(APIView):
    ...
    def get(self, request):
        ...
        if LOAD_RESTORE_PARAM_NAME not in request.GET:
            return Response(
                "A required parameter is missing.", status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if (not upload_id) or (upload_id == ""):
            return Response(
                "An invalid ID has been provided.", status=status.HTTP_400_BAD_REQUEST
            )

        try:
            su = get_stored_upload(upload_id)
        except StoredUpload.DoesNotExist as e:
            LOG.error("StoredUpload with ID [%s] not found: [%s]" % (upload_id, str(e)))
...
class RestoreView(APIView):
    ...
    def get(self, request):
        ...
        response["Content-Disposition"] = "inline; filename=%s" % upload_file_name
...
class FetchView(APIView):
    ...
    def _process_request(self, request):
        ...
        validator = URLValidator(
            message=("An invalid URL <%s> has been " "provided" % (target_url))
        )
        try:
        ...
        try:
            ...
            msg = "Unable to access the requested remote file headers: %s" % str(e)
            LOG.error(msg)
        ...
        if "html" in content_type.lower() and ".html" not in target_url:
            LOG.error(
                "The requested data seems to be in HTML format. "
        ...
        try:
            ...
            raise NotFound("Unable to access the requested remote file: %s" % str(e))
    ...
    def head(self, request):
        ...
        memfile = InMemoryUploadedFile(
            buf, None, file_id, content_type, file_size, None
        )
        tu = TemporaryUpload(
            upload_id=upload_id,
            file_id=file_id,
