...
from django_drf_filepond.models import temp_storage
...
class DrfFilepondChunkedUploadedFile(UploadedFile):
    def __init__(self, temp_chunked_upload_model_obj, content_type=None):
        ...
        self.chunk_dir = os.path.join(
            temp_storage.base_location, temp_chunked_upload_model_obj.upload_dir
        )
        ...
        self.first_file = os.path.join(self.chunk_dir, "%s_1" % (self.chunk_base))
        ...
        for i in range(2, self.num_chunks + 1):
            chunk_file = os.path.join(self.chunk_dir, "%s_%s" % (self.chunk_base, i))
            if not os.path.exists(chunk_file):
                raise FileNotFoundError("Chunk file not found for chunk <%s>" % (i))
    ...
    def size(self):
        if os.path.exists(self.chunk_dir):
            ...
            try:
                ...
                raise AttributeError("Unable to get the file's size: %s" % str(e))
            return size
    ...
    def chunks(self, chunk_size=None):
        ...
        if self.closed:
            raise OSError(
                'File must be opened with "open(mode)" before '
        ...
        if self.current_chunk == 1:
            try:
                ...
                LOG.debug(
                    "AttributeError or UnsupportedOperation when " "trying to seek to 0"
                )
        else:
